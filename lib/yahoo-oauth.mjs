...
export async function verifyYahooFantasyRead(code, config, fetchImpl = fetch) {
    ...
    try {
        ...
        // A successful HTTP read is enough for this POC. Never return or save Yahoo data or tokens.
        ...
        if (fantasyResponse.ok) return { outcome: 'verified' };
        const httpStatus = fantasyResponse.status;
        return Number.isInteger(httpStatus) && httpStatus >= 400 && httpStatus <= 599
            ? { outcome: 'fantasy_http_error', httpStatus }
            : { outcome: 'fantasy_http_error' };
    } catch {
